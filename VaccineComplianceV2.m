...
%                 end
            case mu1arr(2)
                ...
                %figure(2);
                figure(1);subplot(1,2,2);
                plot(tdiscr,1./mu,'LineWidth',4,'color',get(h1(vaccRateCount),'color'),'MarkerEdgeColor',get(h1(vaccRateCount),'color'));hold on;
            otherwise
                figure(1);subplot(1,2,2);
                %figure(2);
                plot(tdiscr,1./mu,'-^','MarkerSize',8,'color',get(h1(vaccRateCount),'color'),'MarkerEdgeColor',get(h1(vaccRateCount),'color'));hold on;
        end
    end
...
legend([h1,h],'Slow vaccination','Fast vaccination','Netherlands data','UK data         ','location','southoutside','interpreter','latex');
...
%grid on;
xlabel('Time (days)','interpreter','latex');
ylabel({'Duration of compliance (days)'},'interpreter','latex');
%legend('Constant','Slow vaccination','Fast vaccination','interpreter','latex');
set(gca,'FontSize',25);

figure(1);annotation('textbox', [0.48, 0.98, 0, 0], 'string', 'A','FontWeight','bold','FontSize',25)
figure(1);annotation('textbox', [0.92, 0.98, 0, 0], 'string', 'B','FontWeight','bold','FontSize',25)
