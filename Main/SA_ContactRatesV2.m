...
% to parameters: the average contact rate at the start of the simulation
% altogether we will have 6 scenarios: 3 contact rates settings, 2 virus
...
CRBase=5;
%for each CRBase calculate c and r1
...
crarr=[4,5,6];
factorarr=crarr/CRBase;
CRarr=factorarr*c;
betaArr=CRarr*epsilonarr(1);
Re1arr=betaArr*S0./(gamma*(N0+r1*Nc0))+betaArr*r1*Sc0.*(mu0*(alpha+gamma+mu0)+alpha*gamma*r1)./(gamma*(alpha+mu0)*(gamma+mu0).*(N0+r1*Nc0));
...
legend(h1(2,:),'Low contact rate','Medium contact rate','High contact rate','numColumns',3,'location','southoutside','interpreter','latex');
xlabel('Time (days)','interpreter','latex');
set(gca,'FontSize',25);
