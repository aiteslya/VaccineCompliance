...
% altogether we will have 12 scenarios: 4 seroprevalence settings, 2 virus
% settings, 12 figures
%prepare settings
clc;
clear variables;
...
SP=0.08;
PerCompl=0.65;
TotalInfarr=[37706,82000,113000,145000];
%total population
N=1.7e7;
popOut=1e5;
XpopOut=N/popOut;
...
for TotalInf=TotalInfarr
    ...
    for j=1:2
        ...
        xline(365,'-.','color',yearcol,'LineWidth',2);
        xline(730,'-.','color',yearcol,'LineWidth',2);
        legend(h1(j,:),'0.22\% prevalence','0.48\% prevalence','0.66\% prevalence','0.85\% prevalence','numColumns',4,'location','southoutside','interpreter','latex');
        xlabel('Time (days)','interpreter','latex');
        set(gca,'FontSize',25);
