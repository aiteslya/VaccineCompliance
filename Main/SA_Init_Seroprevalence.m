...
% altogether we will have 6 scenarios: 3 seroprevalence settings, 2 virus
% settings, 12 figures
%prepare settings
clc;
clear variables;
...
for SP=SParr
    ...
    for j=1:2
        ...
        xline(365,'-.','color',yearcol,'LineWidth',2);
        xline(730,'-.','color',yearcol,'LineWidth',2);
        legend(h1(j,:),'5\% seroprevalence','8\% seroprevalence','15\% seroprevalence','30\% seroprevalence','numColumns',4,'location','southoutside','interpreter','latex');
        xlabel('Time (days)','interpreter','latex');
        set(gca,'FontSize',25);
